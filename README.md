# 🛡️ Cyber-TIL — Today I Learned (Cybersecurity)

![Cybersecurity](https://img.shields.io/badge/Topic-Cybersecurity-red?style=for-the-badge&logo=hackthebox)
![Status](https://img.shields.io/badge/Status-Active-brightgreen?style=for-the-badge)
![Daily](https://img.shields.io/badge/Frequency-Daily-blue?style=for-the-badge)

> 🔐 One cybersecurity concept. One file. Every single day.

---

## 📁 Structure of Each File

| Section | Description |
|---|---|
| 📌 What it is | Simple definition |
| ⚙️ How it works | Brief technical explanation |
| 🌍 Real-world Example | Practical scenario |
| ⚠️ Why it matters | Attacker / Defender perspective |
| 🔑 Key Terms | Related concepts |
| 🛠️ Tip or Tool | Useful command or tool |

---

## 🎯 Goal
Learning cybersecurity from the roots — consistently, practically, and publicly.

---

## 📈 Progress
| Day | Topic |
|---|---|
| Day 01 | ARP Spoofing |
| Day 02 | DNS Spoofing |
| Day 03 | SSL Stripping |
| Day 04 | SQL Injection |
| Day 05 | Cross-Site Scripting (XSS) |
| Day 06 | Cross-Site Request Forgery (CSRF) |
| Day 07 | Server-Side Request Forgery (SSRF) |
| Day 08 | Privilege Escalation |
| Day 09 | Reverse Shell |
| Day 10 | Buffer Overflow |
| Day 11 | WPA2 Cracking |
| Day 12 | Evil Twin AP |
| Day 13 | Deauth Attack |
| Day 14 | PMKID Attack |
| Day 15 | Hash Cracking |
| Day 16 | Rainbow Tables |
| Day 17 | Pass the Hash |
| Day 18 | Rootkits |
| Day 19 | Keyloggers |
| Day 20 | Remote Access Trojans |
| Day 21 | MITRE ATT&CK Framework |
| Day 22 | Cyber Kill Chain |
| Day 23 | OWASP Top 10 |
| Day 24 | PTES |
| Day 25 | Metasploit Framework |
| Day 26 | Burp-Suite |
| Day 27 | Cobalt Strike |
| Day 28 | SIEM & Log Analysis |
| Day 29 | Threat Hunting | 
| Day 30 | Incident Response | 
| Day 31 | Blue Team VS Red Team |
| Day 32 | Prompt Injection |
| Day 33 | LLM Jailbreaking |
| Day 34 | Model Poisoning |
| Day 35 | Adversial Examples|
| Day 36 | Social Engineering |
| Day 37 | Phishing |
| Day 38 | Email Phishing |
| Day 39 | Spear Phishing |
| Day 40 | Whaling |
| Day 41 | Smishing | 
| Day 42 | Vishing |
| Day 43 | Clone Phishing |
| Day 44 | BEC |
| Day 45 | Angler Phishing |
| Day 46 | SEO Phishing |
| Day 47 | QR Phishing |
| Day 48 | Pharming |
| Day 49 | Social Media Phishing |
| Day 50 | OAuth Phishing |
| Day 51 | Cloud Misconfigurations |
| Day 52 | s3 Bucket Exposure |
| Day 53 | Iam Privilege Escalation | 
| Day 54 | Cloud Metadata Exploitation |
| Day 55 | Container Security |
| Day 56 | Serverless Security |
| Day 57 | Cloud Native Threats |
| Day 58 | OSINT Techniques & Tools |
| Day 59 | Dark Web Monitoring |
| Day 60 | Indicators of Compromise |
| Day 61 | Threat Actor Profiling |
| Day 62 | Memory Forensics |
| Day 63 | Disk Forensics |
| Day 64 | Log Forensics |
| Day 65 | Malware Analysis |
| Day 66 | Network Forensics |
| Day 67 | Symmetric & Asymmetric Encryption |
| Day 68 | PKI & Digital Certificates |
---
## Topics Covered

| Topic | Days |
|-------|------|
| Network Attacks | Day 01 - 03 |
| Web Application Attacks | Day 04 - 07 |
| System / OS Level Attacks | Day 08 - 10 |
| Wireless Attacks | Day 11 - 14 |
| Cryptography Attacks | Day 15 - 17 |
| Malware & Persistence | Day 18 - 20 |
| Frameworks & Methodologies | Day 21 - 24 |
| Exploitation Framework | Day 25 - 27 |
| Defensive Security | Day 28 - Day 31 |
| AI Security | Day 32 - 35 | 
| Social Engineering & Phishing | Day 36 - 50 |
| Cloud Security | Day 51 - 57 |
| Threat Intelligence | Day 58 - 61 |
| Digital Forensics | Day 62 - 66 |
| Cryptography Fundamentals | Day 67 onwards |

> _"Security is not a product, but a process."_ — Bruce Schneier
